...
describe('tablesInScope', () => {
  ...
  it('finds a schema-qualified table without an alias', () => {
    ...
    expect(refs(doc, doc.length)).toEqual([{ path: ['bookings', 'airports_data'] }]);
  });
  ...
  it('finds comma-separated tables', () => {
    ...
    expect(refs(doc, doc.length)).toEqual([{ path: ['users'] }, { path: ['customers'] }]);
  });
  ...
  it('stops at where', () => {
    const doc = 'select * from users where id = (select max(id) from customers)';
...
describe('schemaColumnSource (in-scope columns)', () => {
  ...
  it('suggests columns in the SELECT list with a qualified FROM and no alias', () => {
    const got = suggestionsAt(extension(), 'select timez from bookings.airports_data', 12);
  ...
  it('qualifies ambiguous column names across in-scope tables', () => {
    const got = suggestionsAt(extension(), 'select * from users, customers where i', 38);
  ...
  it('keeps alias-dot behavior unchanged', () => {
    const got = suggestionsAt(extension(), 'select * from bookings.airports_data ad where ad.', 49);
    expect(got).toEqual(expect.arrayContaining(['airport_code', 'timezone', 'city']));
  });

  it('keeps schema.table. behavior unchanged', () => {
    const got = suggestionsAt(extension(), 'select bookings.airports_data.', 30);
    expect(got).toEqual(expect.arrayContaining(['airport_code', 'timezone', 'city']));
  });

  it('suggests nothing column-ish for a subquery alias', () => {
    const got = suggestionsAt(extension(), 'select * from (select 1) s where em', 35);
    expect(got).not.toContain('email');
  });

  it('works across statements', () => {
    const got = suggestionsAt(extension(), 'select 1; select * from users where em', 38);
    expect(got).toContain('email');
  });
});
