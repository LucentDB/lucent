...
/**
 * Base width of the row-number gutter (`td.row-num` / `th.row-num`) for 1-2 digits.
 * The gutter renders inside the sticky start region but outside the table's pinning
 * model, so every pinned-column offset must include it or the first pinned
 * data column slides under the gutter.
 */
export const GUTTER_WIDTH: number = 34;

/**
 * Computes the required row-number gutter width (`td.row-num` / `th.row-num`)
 * based on the maximum row index. Base width is 34px for 1-2 digits (fits checkbox
 * & 2 digits), expanding with +8px per additional digit (42px for 3 digits,
 * 50px for 4 digits, 58px for 5 digits) so numbers never bleed over column borders.
 */
export function computeGutterWidth(maxRow: number): number {
  const digits = Math.max(2, String(Math.max(1, maxRow)).length);
  return GUTTER_WIDTH + Math.max(0, digits - 2) * 8;
}
