...
describe('pinned offsets', () => {
  ...
  it('offsets the first left-pinned column past dynamic gutter widths', () => {
    const { container } = renderHeader({ pinLeft: ['0'], gutterWidth: 50 });
    const idTh = [...container.querySelectorAll('thead th')].find((el) =>
      el.textContent?.includes('id'),
    );
    const gutter = container.querySelector('th.row-num');
    expect(gutter?.getAttribute('style')).toContain('width: 50px');
    expect(idTh?.getAttribute('style')).toContain('left: 50px');
  });
