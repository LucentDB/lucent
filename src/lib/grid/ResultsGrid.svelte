<script>
  ...
  import { createGridEngine, computeGutterWidth } from './engine.svelte.ts';
  ...
  let lastTabId = $state(untrack(() => tabId));

  const maxRowNumber = $derived(
    Math.max(
      stream.pageRows.length
        ? stream.page * pageSize + stream.pageRows.length
        : 0,
      fetchedCount,
      totalCount ?? 0,
      1,
  ...
  const gutterWidth = $derived(computeGutterWidth(maxRowNumber));
  ...
  // Keep the table width exactly matching the sum of column widths plus gutter so
  ...
  $effect(() => {
    ...
    const total =
      gutterWidth + columns.reduce((sum, _, i) => sum + getColWidth(i), 0);
