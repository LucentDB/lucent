...
describe('pinned cell offsets', () => {
  ...
  it('offsets left-pinned cells past dynamic gutter widths', () => {
    const { container } = renderBody({ pinLeft: ['0'], gutterWidth: 50 });
    const firstRow = container.querySelectorAll('tbody tr')[0];
    const startCells = firstRow.querySelectorAll('.cell-start');
    expect(startCells[0].getAttribute('style')).toContain('width: 50px');
    expect(startCells[1].getAttribute('style')).toContain('left: 50px');
  });
