...
mod integration {
    ...
    fn guard_full_pipeline() {
        assert!(guard::validate_readonly("SELECT * FROM users").is_ok());
        assert!(guard::validate_readonly("WITH x AS (SELECT * FROM o) SELECT * FROM x").is_ok());
        assert!(guard::validate_readonly("DELETE FROM users WHERE id=1").is_err());
        assert!(guard::validate_readonly("EXPLAIN ANALYZE SELECT * FROM users").is_err());
        assert!(guard::validate_readonly("SELECT 1; SELECT 2").is_err());
        assert!(guard::validate_readonly("INSERT INTO t VALUES (1)").is_err());
    }

    #[test]
    fn blast_radius_extraction() {
        let sql = "DELETE FROM orders WHERE status='cancelled' AND created_at<'2024-01-01'";
        let c = guard::extract_where_for_count(sql).unwrap();
        assert!(c.contains("status") && c.contains("created_at"));
    }

    #[test]
    fn blast_radius_no_where_returns_none() {
        assert!(guard::extract_where_for_count("DELETE FROM orders").is_none());
    }

    #[test]
    fn extract_table_name() {
        assert_eq!(
            guard::extract_table_name("DELETE FROM orders WHERE id=1").as_deref(),
            Some("orders")
        );
        assert_eq!(
            guard::extract_table_name("UPDATE users SET a=1 WHERE id=1").as_deref(),
            Some("users")
        );
    }
    ...
    fn small_schema_verbose() {
        let p = build_system_prompt(&schema(2), None, false);
